...
export default function BookDetail() {
  ...
  useEffect(() => {
    ...
    async function fetchReviews() {
      try {
        const { data: allReviews } = await api.get(`/avaliacoes/livro/${id}`)
        if (isMounted) {
          setReviews(allReviews)
        }
        const { data: userReview } = await api.get(`/avaliacoes/me/livro/${id}`).catch(() => ({ data: null })); // Handle 404 for no user review
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      if (myReview && editMode) {
        await api.put(`/avaliacoes/livro/${id}`, reviewPayload);
      } else {
        await api.post('/avaliacoes', reviewPayload);
      }
      
      // Re-fetch reviews to update the list
      const { data: updatedReviews } = await api.get(`/avaliacoes/livro/${id}`);
      setReviews(updatedReviews);
      const { data: updatedMyReview } = await api.get(`/avaliacoes/me/livro/${id}`);
