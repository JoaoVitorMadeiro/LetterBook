...
export default function Books() {
  ...
  useEffect(() => {
    async function fetchAuthors() {
      try {
        const { data } = await api.get('/autores', { params: { size: 1000 } }) // Fetch all authors
  ...
  useEffect(() => {
    async function fetchGenres() {
      try {
        const { data } = await api.get('/generos', { params: { size: 1000 } }) // Fetch all genres
  ...
  const handleBookClick = (bookId) => {
    router.push(`/books/${bookId}`) // Navigate to a hypothetical book detail page
  }
