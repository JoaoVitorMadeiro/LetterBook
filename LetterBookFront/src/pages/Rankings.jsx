  ...
  useEffect(() => {
    ...
    async function fetchRankings() {
      ...
      try {
        ...
        const { data } = await api.get('/api/v1/rankings/global', { params })
        if (isMounted) {
        ...
        <Paper>
          <List>
            {ranking.map((user, idx) => (
              <Box key={user.userId}>
                <ListItem alignItems="flex-start" component={Link} to={`/profile/${user.userId}`} sx={{ textDecoration: 'none', color: 'inherit' }}>
