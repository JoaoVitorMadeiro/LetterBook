...
export default function Feed() {
  ...
  const handleLike = async (avaliacaoId) => {
    try {
      await api.post(`/api/v1/feed/avaliacoes/${avaliacaoId}/curtir`)
  ...
  const handleSubmitComment = async (avaliacaoId) => {
    ...
    try {
      await api.post('/api/v1/feed/comentarios', { avaliacaoId, comentario })
            ...
            <Card key={item.id} sx={{ mb: 3 }}>
              <CardContent>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  ...
                  <Box>
                    <Typography variant="subtitle1" component={Link} to={`/profile/${item.usuario?.id}`} sx={{ textDecoration: 'none', color: 'inherit', fontWeight: 'bold' }}>
                      {item.usuario?.nome}
                    </Typography>
                ...
                {item.tipo === 'REVIEW' && (
                  <Box>
                    <Typography variant="body1" component="div">
                      avaliou o livro{' '}
                      <Typography component={Link} to={`/books/${item.book?.id}`} sx={{ textDecoration: 'none', fontWeight: 'bold' }}>
                        {item.book?.titulo}
                      </Typography>
                    </Typography>
