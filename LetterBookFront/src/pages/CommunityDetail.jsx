...
export default function CommunityDetail() {
  ...
  const fetchCommunityDetails = async () => {
    setLoading(true)
    try {
      const { data } = await api.get(`/api/v1/comunidades/${id}`)
      setCommunity(data)
      setIsMember(data.isMember) // Assuming backend tells us if user is a member

      const { data: membersData } = await api.get(`/api/v1/comunidades/${id}/membros`)
      setMembers(membersData)

      const { data: rankingData } = await api.get(`/api/v1/comunidades/${id}/ranking`)
  ...
  const handleJoinLeaveCommunity = async () => {
    try {
      if (isMember) {
        await api.delete(`/api/v1/comunidades/${id}/sair`)
        setIsMember(false)
      } else {
        await api.post(`/api/v1/comunidades/${id}/entrar`)
        setIsMember(true)
      }
